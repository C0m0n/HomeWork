...
public class EventsDemo extends Application {
	...
	private boolean lVis = false;
	private boolean tVis = false;
	...
	public void start(Stage primaryStage) throws Exception {
		...
		miCircle.setOnAction((e) -> {
			circle.setVisible(!cVis);
			cVis = !cVis;
		});
		miLine.setOnAction((e) -> {
			line.setVisible(!lVis);
			lVis = !lVis;
		});
		miTriangle.setOnAction((e) -> {
			triLineBottom.setVisible(!tVis);
			triLineLeft.setVisible(!tVis);
			triLineRight.setVisible(!tVis);
			tVis = !tVis;
		});
		miShowAll.setOnAction((e)-> {
			showAll(true);
		});
		miClearAll.setOnAction((e)->{
			showAll(false);
		});
	...
	private void showAll(boolean tF) {
		circle.setVisible(tF);
		line.setVisible(tF);
		triLineBottom.setVisible(tF);
		triLineLeft.setVisible(tF);
		triLineRight.setVisible(tF);
	}

	private void checkAnswer(String text){
		if (text.equalsIgnoreCase("Hairy Potter")){
