...
// Initialize the cache arrays containing the tags and the blocks
	...
	cacheArray = new int[number_of_lines_in_cache, number_of_words_in_block];
...
// Your Code// 
    int numberOfWordBits = log2(number_of_words_in_block);
    ...
    line = (address & line_mask) >> numberOfWordBits;
...
unsigned int DirectCache::getPercentageOfHits(void) {
    ...
    return (unsigned int)(((float)number_of_hits / (float)number_of_memory_requests) * 100);
}
